...
describe('UrlParametersService', () => {
  ...
  beforeEach(async(() => {
    TestBed.configureTestingModule({
      imports: [HttpModule, StoreModule.forRoot({}), RouterTestingModule],
      declarations: [],
      providers: [
  ...
  it('ActiveImage', () => {
    ...
    const count = combinePerPage.calls.count();
  ...
  it('getParamsStringForPage()', () => {
    const defaultParams = urlParametersService.getParamsStringForPage('unknow');

    expect(defaultParams).toEqual('');

    urlParametersService.dispatchToStore({ countries: ['newCountry', 'oneMore'], zoom: '6' });
    const newRow = urlParametersService.getParamsStringForPage('/matrix');
