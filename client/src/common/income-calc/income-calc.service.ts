...
import { assign, filter, get, map } from 'lodash-es';
...
export class IncomeCalcService {
  ...
  calcPlaceIncome(income: number, timeUnit: string, currencyValue): string {
    const currencyIncome = this.calcIncomePerTime(income, timeUnit, currencyValue);

    return this.math.roundIncome(currencyIncome, currencyIncome <= 10)
      .toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ');
  }

  calcIncomePerTime(income: number, timeUnit: string, currencyValue): number {
    let resultIncome = 0;
    ...
    const currencyIncome = resultIncome * currencyValue;
