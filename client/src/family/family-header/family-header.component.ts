...
import { DEBOUNCE_TIME, TIME_UNIT_CODES } from '../../defaultState';
...
export class FamilyHeaderComponent implements OnInit, OnDestroy {
  ...
  maxHeightPopUp: number = this.windowHeight * 0.95 - 91;
  ...
  ngOnInit(): void {
    ...
    this.getTranslationSubscribe = this.languageService
      .getTranslation(['READ_MORE', 'READ_LESS'])
      .subscribe((trans: any) => {
    ...
    this.resizeSubscribe = fromEvent(window, 'resize')
      ...
      .subscribe(() => {
        this.zone.run(() => {
          ...
          this.maxHeightPopUp = this.windowHeight * 0.95 - 91;
        });
      });
  }
  ...
  getFamilyHeaderData(): void {
    if (this.placeId) {
      ...
      this.familyHeaderServiceSubscribe = this.familyHeaderService.getFamilyHeaderData(query).subscribe(
        (res: any): any => {
  ...
  closeAboutDataPopUp(event: MouseEvent | KeyboardEvent): void {
  ...
  truncCountryName(countryData: any): any {
    switch (countryData.alias) {
      ...
      case 'United Kingdom':
        ...
        break;
      default:
        this.countryName = countryData.alias.length > 10 ? `${countryData.alias.slice(0, 8)}...` : countryData.alias;
    }
  }
  ...
  private calcIncomeValue(): void {
    if (this.timeUnit && this.currencyUnit && this.home) {
      this.familyIncome = this.incomeCalcService.calcPlaceIncome(
        this.home.income,
        this.timeUnit.code,
