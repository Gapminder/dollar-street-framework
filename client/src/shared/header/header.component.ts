...
import { forEach, difference, map, find, get, assign } from 'lodash-es';
import {
  Component,
  Input,
  Output,
...
export class HeaderComponent implements OnDestroy, AfterViewInit, OnInit {
  @ViewChild(ThingsFilterComponent) thingsFilterComponent: ThingsFilterComponent;
  @ViewChild(CountriesFilterComponent) countriesFilterComponent: CountriesFilterComponent;
  @ViewChild('filtersContainer') filtersContainer: ElementRef;
  @ViewChild('incomeTitleContainer') incomeTitleContainer: ElementRef;

  @ViewChild('headerTitle')
  set controlElRef(elementRef: ElementRef) {
  ...
  translations: string[];
  isHeaderFiltersReversed: boolean;
  ...
  ngAfterViewInit(): void {
    ...
    this.ngSubscriptions.scroll = fromEvent(document, 'scroll').subscribe(() => {
  ...
  ngOnInit(): void {
    ...
    this.ngSubscriptions.getTranslation = this.languageService
      .getTranslation(['BY_INCOME', 'THE_WORLD'])
    ...
    this.ngSubscriptions.combileTranslations = fromEvent(
      this.languageService.translationsLoadedEvent,
      this.languageService.translationsLoadedString
    ).subscribe((trans: TranslationsInterface) => {
      this.getTranslations(trans);
    });
    ...
    this.ngSubscriptions.languageState = languageState
      ...
      .subscribe((language: LanguageState) => {
        ...
        // "B {{ 'of' | translate }} A".
        this.isHeaderFiltersReversed = this.urlParams.lang === 'ja';
      });
    ...
    this.ngSubscriptions.matrixState = matrixState.debounceTime(DEBOUNCE_TIME).subscribe((matrix: MatrixState) => {
      ...
      if (get(matrix, 'currencyUnits', false) && this.currencyUnits !== matrix.currencyUnits) {
        this.currencyUnits = this.incomeCalcService.addCurrencyNames(
        ...
        if (!this.currencyUnit) {
          if (this.urlParams.currency) {
            this.currencyUnitTemp = this.incomeCalcService.getCurrencyUnitByCode(
              this.currencyUnits,
              this.urlParams.currency,
              this.languageService.currentLanguage
            );
          } else {
            this.currencyUnitTemp = this.incomeCalcService.getCurrencyUnitForLang(
              this.currencyUnits,
              this.languageService.currentLanguage
            );
          }
  ...
  isCurrentPage(name: string): boolean {
    const shap = this.activatedRoute.snapshot.root.children
      .map((child) => child.url)
  ...
  goToMatrixFromFamilyPage(): void {
    this.store.dispatch(new MatrixActions.SetPlace(null));
    ...
    if (regions[0] !== 'World') {
      if (regions.length > 2) {
        ...
        if (getDifference.length) {
          title =
            getDifference.length === 1 && regions.length === 1
              ? getTranslatedRegions[0] + ' & ' + getDifference[0]
  ...
  translateTimeUnit(): void {
    if (this.timeUnitTemp) {
      ...
      this.selectedTimeInterval = this.timeUnitTemp.code;
    }
  }
}
