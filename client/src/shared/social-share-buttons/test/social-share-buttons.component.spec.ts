...
describe('SocialShareButtonsComponent', () => {
  ...
  it('open twitter popup', () => {
    spyOn(languageService, 'getTranslation').and.returnValue(
      Observable.of({ SEE_HOW_PEOPLE: 'see', REALLY: 'really', LIVE: 'live' })
    );
    ...
    expect(component.newWindow.location.href).toEqual(
      'https://twitter.com/intent/tweet?url=/expected/url&text=see%20really%20live-%20Dollar%20Street'
    );
  });
  ...
  it('open facebook popup', () => {
    ...
    expect(component.socialShareButtonsServiceSubscribe).toBeDefined();
    expect(component.newWindow.location.href).toEqual(
      'http://www.facebook.com/sharer.php?u=/expected/url&description=see%20really%20live'
    );
  });
  ...
  it('open linkedin popup', () => {
    ...
    expect(component.socialShareButtonsServiceSubscribe).toBeDefined();
    expect(component.newWindow.location.href).toEqual(
      'http://www.linkedin.com/shareArticle?mini=true&url=/expected/url&summary=see%20really%20live'
    );
  });
});
