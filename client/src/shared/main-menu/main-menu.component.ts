...
import { Component, OnInit, OnDestroy, HostListener, ElementRef, AfterViewInit, ViewChild, Input } from '@angular/core';
...
import { PagePositionService } from '../page-position/page-position.service';
...
export class MainMenuComponent implements OnInit, OnDestroy, AfterViewInit {
  @ViewChild('socialShareContent') socialShareContent: ElementRef;
  ...
  isMatrixPage = false;
  MAX_LENGHT_SHARE_TRANSLATION = 10;
  ...
  processShareTranslation(): void {
    ...
    if (this.socialShareContentElement) {
      ...
      if (this.shareTranslation.length > this.MAX_LENGHT_SHARE_TRANSLATION) {
  ...
  goToMatrixPage(): void {
    ...
    this.store.dispatch(new MatrixActions.UpdateActiveHouse({ row: 1, index: undefined }));
