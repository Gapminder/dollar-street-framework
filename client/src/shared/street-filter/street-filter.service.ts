...
import { Place, Currency, SubscriptionsList, DividersGaps, TimeUnit, IncomeFilter } from '../../interfaces';
...
export class StreetFilterDrawService {
  ...
  isDrawDividers(drawDividers: DrawDividersInterface): this {
    ...
    this.svg
      ...
      .attr('class', (d) => {
        return `scale-label${this.calculateIncomePerTimeUnit(d)}`;
      })
      .attr('y', this.height)
      .attr('fill', '#767d86');
  ...
  drawScale(places, drawDividers): this {
    ...
    this.svg
      ...
      .attr('points', () => {
        ...
        const point3 = `${this.width + this.halfOfStreetOffset},${this.halfOfHeight - 4}`;
    ...
    if (get(this.ngSubscribtions, 'touchMove', false)) {
      this.ngSubscribtions.touchMove.unsubscribe();
    }

    this.ngSubscribtions.touchMove = fromEvent(window, 'touchmove', { passive: true }).subscribe((e: TouchEvent) => {
      const touch: Touch = e.touches[0];

      this.onMouseEvent(touch);
    });

    if (get(this.ngSubscribtions, 'mouseMove', false)) {
      this.ngSubscribtions.mouseMove.unsubscribe();
    }

    this.ngSubscribtions.mouseMove = fromEvent(window, 'mousemove', { passive: true }).subscribe((e: MouseEvent) => {
      this.onMouseEvent(e);
    });

    if (get(this.ngSubscribtions, 'touchUp', false)) {
      this.ngSubscribtions.touchUp.unsubscribe();
    }

    this.ngSubscribtions.mouseUp = fromEvent(window, 'mouseup', { passive: true }).subscribe(() => {
    ...
    this.ngSubscribtions.touchUp = fromEvent(window, 'touchend', { passive: true }).subscribe(() => {
      if (!this.sliderLeftMove && !this.sliderRightMove && !this.draggingSliders) {
        return;
      }
  ...
  drawLeftSlider(x: number, init = false): this {
    ...
    this.leftScroll.attr('x', x > 0 ? x : 0);

    this.leftScrollOpacityStreet.attr('width', () => {
      const width = x + this.halfOfStreetOffset;

      return width > 0 ? width : 0;
    });
    this.leftScrollOpacityHomes.attr('width', () => {
      const width = x + this.halfOfStreetOffset;
  ...
  drawRightSlider(x: number): this {
    ...
    this.rightScroll.attr('x', x);

    this.rightScrollOpacityStreet
      .attr('x', x + this.halfOfStreetOffset)
  ...
  drawScrollLabel(): this {
    ...
    this.leftScrollText.text(`${this.currencyUnit.symbol}${incomeL}`).attr('x', () => {
  ...
  calculateIncomePerTimeUnit(income) {
