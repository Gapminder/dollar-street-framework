...
import { MatrixImagePreview } from './matrix-image-preview.e2e.component';
import { FamilyImagePreview } from './family-image-preview.e2e.component';
import { MatrixPage } from '../matrix.page';
import { scrollIntoView, waitForVisible } from '../../helpers/common-helper';
...
export class FamilyImage {
  ...
  constructor(url: string, index: number) {
    if (url === MatrixPage.url) {
      // TODO: Remove Matrix Page usage form component to avoid circular dependency
      this.type = 'matrix';
  ...
  async openPreview(): Promise<any> {
    await waitForVisible(this.rootSelector, 10000);
    await scrollIntoView(this.rootSelector);
