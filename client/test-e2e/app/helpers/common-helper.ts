...
export function getRandomNumber(): number {
  ...
  const randomNumber = Math.ceil(Math.random() * 14); // random number between 1 and 15
...
export async function waitForTextPresent(element: ElementFinder, value: string, timeout = 5000): Promise<void> {
  await browser.wait(EC.textToBePresentInElement(element, value), timeout);
}
...
export function isInViewport(element: ElementFinder): promise.Promise<boolean> {
  return browser.executeScript((elementToCheck) => {
    ...
    return (
      rect.top >= 0 &&
      rect.left >= 0 &&
      rect.bottom * 0.9 <= (window.innerHeight || html.clientHeight) &&
