angular.module('angularUtils.directives.uiBreadcrumbs', ['ui.router'])
  ...
  .directive('uiBreadcrumbs', ['$interpolate', '$state', '$stateParams', '$http', function ($interpolate, $state, $stateParams, $http) {
    return {
      restrict: 'E',
      templateUrl: '/view/uiBreadcrumbs.tpl.html',
      scope: {
        displaynameProperty: '@',
      ...
      link: function (scope, element, attrs) {
        ...
        function updateBreadcrumbsArray() {
          ...
          if (scope.breadcrumbs[2]) {
            if (scope.breadcrumbs[2].route === "frontend.app.places.place") {
              scope.breadcrumbs[2].displayName = $stateParams.placeId.replace("-slash-", "/");
            }
            if (scope.breadcrumbs[2].route === "frontend.app.things.thing") {
              scope.breadcrumbs[2].displayName = $stateParams.name;
              $http.get('/things').success(function (data) {
                scope.breadcrumbs[2].things = data.data
                data.data.forEach(function (thing) {
                  if (thing.thingName === scope.breadcrumbs[2].displayName) {
                    scope.breadcrumbs[2].thing = thing
                  }
                })
                scope.breadcrumbs[2].choseThing = function (thing) {
                  $state.go('frontend.app.things.thing', {name: thing.thingName});
                }
              })
